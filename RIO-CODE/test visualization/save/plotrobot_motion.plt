...
plot robotfile using 1:($9*1000) axes x1y1 with line title "In Auto" lw 3 lc rgb "red", \
	 ...
	 robotfile using 1:($10*1000) axes x1y1 with points title "Left Motor Voltage" lw 3 lc rgb "brown", \
	 robotfile using 1:($11*1000) axes x1y1 with points title "Right Motor Voltage" lw 3 lc rgb "black", \
	 robotfile using 1:($12*1000) axes x1y1 with points title "Talon Voltage" lw 3 lc rgb "blue", \
	 robotfile using 1:($13*1) axes x1y1 with points title "Setpoint1" lw 3 lc rgb "red", \
	 robotfile using 1:($14*1) axes x1y1 with points title "Feedforward1" lw 3 lc rgb "brown", \
	 robotfile using 1:($15*1000) axes x1y1 with points title "Feedback1" lw 3 lc rgb "blue", \
	 robotfile using 1:($16*1000) axes x1y1 with points title "Velocity1" lw 3 lc rgb "purple", \
